...
// 发 Web Push（Cloudflare 用 web-push 兼容方式）
async function sendWebPush(device, npcName, npcId, text, env) {
  // Cloudflare Workers 支持直接用 fetch 发 Web Push
  // 使用 VAPID 签名
  const payload = JSON.stringify({ npcId, npcName, text });

  try {
    // 动态引入 web-push（Cloudflare 支持 npm 包）
    ...
    console.warn('[push] 发送失败:', err.message);
    return false;
  }
}
  ...
  try {
    ...
    for (const uid of Object.keys(byUid)) {
      ...
      if (!apiCfg || !apiCfg.api_key) {
        results.push({ uid, skipped: true, reason: '用户未开启后台推送或未配置 API' });
        continue;
      }
      ...
      for (const npc of byUid[uid]) {
        ...
        for (const dev of (devices || [])) {
          const result = await sendWebPush(dev, npc.npc_name, npc_id, text, env);
          if (result === 'expired') {
