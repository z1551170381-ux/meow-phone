...
//
// ★ v2 修复点：
//   1) Apple Web Push (web.push.apple.com) 必须用 RFC 8291 aes128gcm 加密 payload
//      原版直接发 JSON 明文 → BadWebPushRequest；现在统一走加密，无加密密钥时退化为空 body ping
//   2) AI 上下文质量提升：对话历史作为 messages 多轮注入（而非塞进 system），效果更真实
//   3) 从 meow_chat_history 表读取真实近期对话（表不存在时静默降级）
...
async function sbSelect(env, table, filters, extra = '') {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`sbSelect ${table} ${resp.status}: ${e.slice(0, 200)}`);
  }
  return resp.json();
}
...
async function sbInsert(env, table, data) {
  ...
  if (!resp.ok) {
    const e = await resp.text();
    throw new Error(`sbInsert ${table} ${resp.status}: ${e.slice(0, 200)}`);
  }
}
...
async function sbUpsert(env, table, data, onConflict) {
  ...
  if (!resp.ok) {
    const e = await resp.text();
    throw new Error(`sbUpsert ${table} ${resp.status}: ${e.slice(0, 200)}`);
  }
}
...
async function sbDelete(env, table, filters) {
  ...
  if (!resp.ok) {
    const e = await resp.text();
    throw new Error(`sbDelete ${table} ${resp.status}: ${e.slice(0, 200)}`);
  }
}

// ─────────── 真实近期对话记录 ───────────
// 尝试读 meow_chat_history；表不存在时静默降级
async function fetchRecentChatHistory(env, uid, npcId, limit = 14) {
  try {
    ...
    return rows
      .reverse()
      .map(r => ({
        role: String(r.role || 'npc'),
        text: cleanText(r.text || r.content || '', 120)
      }))
      .filter(r => r.text);
  } catch (_) {
    return []; // 表不存在时静默忽略
...
async function generateMessage(npc, kind, apiCfg, ctx) {
  ...
  const bondMap = {
    '亲近': '关系较近，可以自然关心、分享细节，语气真实但不要油腻。',
    '暧昧': '允许有试探和在意，说话带一点心意但不要像模板情话。',
    '疏远': '语气克制，不要突然过热，也不要太客气。',
  ...
  const contextBlock = [
    ...
    ctx.npcState    ? `【角色当前状态】${ctx.npcState}` : '',
    ctx.userState   ? `【用户当前状态】${ctx.userState}`: '',
  ...
  const systemPrompt = [
    `你正在扮演「${npc.npc_name}」，现在要主动发一条消息给对方。`,
    ...
    '【你要做的事】',
    kind === 'daily'
      ? '今天自然想起对方，顺手发一条——可以是你今天的事、看到的东西、忽然想问的问题，但必须扎根在上面的细节里。'
      : '某个瞬间联想到对方，想说一句——要具体，不要空泛。',
    '',
    '【必须遵守】',
    '- 只输出 1 条消息，不要解释，不要引号，不要署名。',
    '- 语气像正在进行中的关系，延续聊天记录里已有的语感和话题走向。',
    '- 优先引用上面的场景、状态、总结、最近话题中的具体细节；有对话历史就接着那个感觉说。',
    '- 禁止模板句：「在忙吗」「刚忙完顺便问你」「突然想起你」「你在干嘛呢」「最近怎么样」。',
    '- 禁止客服腔、播报腔、打卡腔、营业腔。',
    '- 不要刻意端水式关心，不要连续问两个问题。',
    '- 字数 8-40 字优先，最多 50 字。宁可短，也不要像文案。',
    '- 和聊天记录里角色最近说过的话避免同句式、同开头重复。',
    '- 允许留白、停顿、口语感，可以发表情或简短感叹，但不要故作矫情。'
  ].filter(Boolean).join('\n');

  // ★ 将真实对话历史作为 messages 多轮注入（比放在 system 里效果更真实）
  ...
  const messages = [
    ...
    ...historyMessages,
    { role: 'user', content: '请直接输出那条消息。它应该像这个角色本人在此刻真的会发出来的话。' }
  ];
  ...
  if (!resp.ok) {
    const e = await resp.text();
    throw new Error(`AI ${resp.status}: ${e.slice(0, 200)}`);
  }
...
async function makeVapidJWT(env, audience) {
  ...
  if (d.length !== 32)                  throw new Error(`Bad private key length: ${d.length}`);
  if (pub.length !== 65 || pub[0] !== 0x04) throw new Error(`Bad public key format: ${pub.length}`);
...
async function encryptPayload(plaintextStr, p256dhB64u, authB64u) {
  ...
  const clientPub  = b64uDec(p256dhB64u); // 65 bytes uncompressed
  ...
  const prk   = await hkdf(sharedBits,  concatU8(enc.encode('WebPush: info\x00'), clientPub, serverPubRaw), 32);
  const cek   = await hkdf(prk, enc.encode('Content-Encoding: aes128gcm\x00'), 16);
  const nonce = await hkdf(prk, enc.encode('Content-Encoding: nonce\x00'),     12);

  const aesKey    = await crypto.subtle.importKey('raw', cek, { name:'AES-GCM' }, false, ['encrypt']);
  const plainPad  = concatU8(enc.encode(plaintextStr), new Uint8Array([2])); // padding delimiter
  const ciphertext = await crypto.subtle.encrypt({ name:'AES-GCM', iv:nonce }, aesKey, plainPad);

  // RFC 8291 record: salt(16) + rs(4=4096) + keylen(1) + serverPub(65) + ciphertext
  return concatU8(
    salt,
    new Uint8Array([0x00, 0x10, 0x00, 0x00]), // record size 4096
...
async function sendWebPush(device, npcName, npcId, text, env) {
  try {
    ...
    const payloadJson = JSON.stringify({
      ...
      title:npcName, body:text, text, ts:Date.now()
    });
  ...
  try {
    ...
    for (const uid of Object.keys(byUid)) {
      ...
      for (const npc of byUid[uid]) {
        ...
        let text = '';
        try {
          // ★ 读取真实对话历史（meow_chat_history 表，不存在时降级为空）
          const chatHistory = await fetchRecentChatHistory(env, uid, npc_id, 14);
          const msgCtx = buildMessageContext(npc, chatHistory, now);
        ...
        await sbInsert(env, 'meow_pending_messages', {
          uid, npc_id, npc_name: npc.npc_name, text, kind,
          ts: now, is_pulled: false
        });
        ...
        for (const dev of (devices || [])) {
          ...
          pushDebug.push({
            host, result: pushRes.result, status: pushRes.status,
            text: pushRes.text, endpoint: String(dev.endpoint || '').slice(0, 80),
            encrypted: !!(dev.p256dh && dev.auth)
          });
