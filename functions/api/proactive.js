...
function looksIncompleteMessage(text) {
  ...
  // 很像“刚才在… / 楼下那家… / 今天这个…”这种没说完
  if (/^(刚才|刚刚|刚从|楼下|那家|这个|那个|今天|刚路过|刚在|就在|刚点|刚吃|刚健身|刚出门|刚下楼)/.test(s) && s.length <= 16) {
    return true;
  }

  // 没有任何句末语气，且整体又很短
  if (!/[。！？~～…]$/.test(s) && s.length <= 14) return true;
...
async function generateMessage(npc, kind, apiCfg, ctx) {
  ...
  const systemPrompt = [
    ...
    contextBlock,
    '【核心目标】写出一条适合手机通知直接显示的自然聊天消息。',
    '【必须遵守】',
    '- 只输出 1 条消息，不要解释，不要引号，不要署名。',
    '- 只写 1 句完整的话，优先 16-32 字，最多 40 字。',
    '- 这句话必须单独成立，不能是半句、不能只说一半、不能像备忘关键词。',
    '- 禁止以逗号、顿号、冒号、分号结尾。',
    '- 禁止生成这种残句：如“楼下那家面”“刚才在路边”“刚从健身房出来，”',
    '- 优先引用上面的场景、状态、总结、最近话题中的具体细节；没有细节时也要自然。',
    ...
    const resp = await fetch(`${base_url}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: userPrompt + (extraUserHint ? '\n' + extraUserHint : '') }
        ],
        temperature: 0.88,
...
async function sendWebPush(device, npcName, npcId, text, env) {
  try {
    ...
    if (isApple) {
      // Apple 这条链先完全不要 Topic，继续走“空 push + SW 自己 /api/pull 拉最新消息”
      resp = await postOnce({
