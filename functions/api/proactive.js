...
async function generateMessage(npc, kind, apiCfg, ctx) {
  ...
  // ── 输出校验 ──
  function isValidOutput(t) {
    if (!t || t.replace(/\s/g,'').length < 8) return false;
    // 纯 ASCII 无中文/日文 → 指令泄漏或噪音
    if (/^[\x00-\x7F\s]+$/.test(t) && !/[\u4e00-\u9fff\u3040-\u30ff]/.test(t)) return false;
    // 以规则关键词开头 → 指令泄漏
    const badStart = [
    ...
    for (const p of badStart) if (p.test(t.trim())) return false;
    // ★ 断句检测：中文字符数 < 6 且不以句末标点结尾 → 像未完成的片段
    const cnCount = (t.match(/[\u4e00-\u9fff]/g) || []).length;
    const hasEndPunct = /[。！？…～~!?♪]$/.test(t.trim());
    if (cnCount < 6 && !hasEndPunct) return false;
    return true;
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const cleaned = raw
        .replace(/^[\u201c\u201d\u2018\u2019\u300c\u300e\u3010"'\s]+|[\u201c\u201d\u2018\u2019\u300d\u300f\u3011"'\s]+$/g, '')
        .replace(/^(\u5728\u5fd9\u5417|\u4f60\u5728\u5e72\u5565|\u521a\u5fd9\u5b8c|\u7a81\u7136\u60f3\u8d77\u4f60|\u6700\u8fd1\u600e\u4e48\u6837)[\uff0c\u3002\uff01\uff1f\u3001\s]*/i, '')
        .trim();
      if (isValidOutput(cleaned)) {
        text = cleaned.slice(0, 100);
        break;
      }
      console.warn(`[proactive] attempt ${attempt+1} rejected (len=${cleaned.length}): "${cleaned.slice(0,40)}"`);
    } catch (e) {
